...
export default function ReceivablesTab({ setNotification, refreshData }: ReceivablesTabProps) {
  ...
  const handleDelete = async (tx: any) => {
    ...
    if (!confirm(`Excluir a receita "${tx.description}"?`)) return;

    setDeletingId(tx.id);
    try {
      await deleteTransaction(tx.id);
      setNotification({ msg: "Receita excluída!", type: "success" });
      loadReceivables();
      refreshData();
    } catch {
      setNotification({ msg: "Erro ao excluir receita.", type: "error" });
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setNotification({ msg: "Receita atualizada com sucesso!", type: "success" });
      setEditingTx(null);
      loadReceivables();
      refreshData();
    } catch {
      setNotification({ msg: "Erro ao atualizar receita.", type: "error" });
    } finally {
      setIsEditing(false);
    }
  };
      ...
      {editingTx && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass-card w-full max-w-md p-6 space-y-4 animate-in zoom-in-95 duration-200">
            <h3 className="text-xl font-bold text-white">Editar Receita</h3>
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white outline-none focus:ring-2 focus:ring-emerald-500"
              />
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button 
                ...
                disabled={isEditing}
                className="flex-1 py-3 bg-emerald-500 text-white rounded-xl font-bold hover:bg-emerald-600 transition flex items-center justify-center gap-2"
              >
